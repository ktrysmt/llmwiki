...
skills/
  ingest/                   # /llmwiki:ingest -- 全パイプライン: import + lint + fix
    SKILL.md
...
| スキル | 用途 | 書き込み |
|---|---|---|
| /llmwiki:ingest | 全パイプライン: import + lint + fix | 自動 + 要承認 |
| /llmwiki:lint | 健全性チェック(検出・報告のみ) | なし(読み取り専用) |
| /llmwiki:query | Wikiに自然言語で質問し回答を得る | synthesis保存 + フィードバック(全て自動) |
| /llmwiki:docs | テーマ指定でドキュメント生成 | なし(外部出力のみ) |

## 使い方

### /llmwiki:ingest -- 全パイプライン

```
/llmwiki:ingest              # プロジェクトルートをスキャン
/llmwiki:ingest ~/work/dump  # 外部ディレクトリをスキャン
```
...
- contradictions（「needs review」フラグの件数。/llmwiki:ingest への導線を案内）
...
回答過程で発見された関係性・新規エンティティ・矛盾・dormant promotionは承認なしで自動的にWikiへフィードバックされる。
...
設定は `.llmwiki/config.json` に保存される（初回の `/llmwiki:ingest` 実行時に自動生成）。カスタマイズはこのファイルを直接編集する。
...
- /llmwiki:ingest は genuine 矛盾と decay 降格のみ人間の承認が必要。/llmwiki:query のフィードバックは自動適用
...
- `/llmwiki:ingest` を追加: import → lint → fix を 1 コマンドで実行するパイプライン。決定論的な前処理をフェーズ間で共有
...
- `/llmwiki:ingest` がWiki変更の単一エントリポイント（import + lint + fixパイプライン）
- `/llmwiki:lint` は読み取り専用を維持（検出・報告のみ）。修正提案は `/llmwiki:ingest` へ誘導
